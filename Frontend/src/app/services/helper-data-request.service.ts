...
import { DataRequestDTO, FilterDataRequestDTO, FilterItemDataRequestDTO, SortDataRequestDTO } from "../model/DataRequest.model";
import { THelperArray } from "../tlib/utility/helper-array";


// @dynamic
export class HelperDataRequest {
    ...
    static convertFilterToString(filter: FilterDataRequestDTO) {
        ...
        str = filter.filters.map((f: FilterDataRequestDTO | FilterItemDataRequestDTO) => {
            ...
            if (!THelperArray.hasListValue((f as FilterDataRequestDTO).filters)) {
